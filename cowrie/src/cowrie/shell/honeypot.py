...
class HoneyPotShell:
    ...
    def runCommand(self):
        ...
        for index, cmd in reversed(list(enumerate(cmd_array))):
            ...
            if adaptive.enabled:
                full_cmd = cmd["command"] + " " + " ".join(cmd["rargs"])
                session_id = str(self.protocol.sessionno)
                adaptive.behavior_profiler.analyze_command(session_id, full_cmd, self.protocol.cwd, self.protocol.user.username)
                adaptive.honeytoken_mgr.check_interaction(session_id, full_cmd, self.protocol.cwd)
            ...
            if cmdclass:
                ...
                if adaptive.enabled:
                    adaptive.telemetry_mgr.log_event(
                        ...
                        details="Command not found: {}".format(cmd["command"]),
