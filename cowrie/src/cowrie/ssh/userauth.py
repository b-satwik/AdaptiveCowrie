    ...
    * IP based authentication
    ...
    def auth_publickey(self, packet: bytes) -> Any:
        try:
            ...
            algName, blob, rest = common.getNS(packet[1:], 2)
            pubKey = keys.Key.fromString(blob)
            self.transport.auth_pubkey = pubKey.toString("OPENSSH")
            self.transport.auth_pubkey_fingerprint = pubKey.fingerprint().decode("utf-8", errors="ignore")
        except Exception as e:
            log.msg(f"Failed to capture public key in auth_publickey: {e}")
