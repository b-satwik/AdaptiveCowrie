...
class Command_ps(HoneyPotCommand):
    def call(self) -> None:
        ...
        if adaptive.enabled:
            ...
            procs = adaptive.process_mgr.get_processes(session_user=user, session_cmd=f"ps {args_str}".strip())
            self.write(f"USER       PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND\n")
            for p in procs:
                user_str = p["user"].ljust(10)
                pid_str = str(p["pid"]).rjust(5)
                cpu_str = f"{p['cpu']:.1f}".rjust(5)
                mem_str = f"{p['mem']:.1f}".rjust(5)
                vsz_str = str(p["vsz"]).rjust(7)
                rss_str = str(p["rss"]).rjust(5)
                tty_str = p["tty"].ljust(8)
                stat_str = p["stat"].ljust(5)
                start_str = p["start"].ljust(5)
                time_str = p["time"].rjust(6)
                cmd_str = p["command"]
                self.write(f"{user_str} {pid_str} {cpu_str} {mem_str} {vsz_str} {rss_str} {tty_str} {stat_str} {start_str} {time_str} {cmd_str}\n")
            return
