...
class Command_grep(HoneyPotCommand):
    ...
    def grep_get_contents(self, filename: str, match: str) -> None:
        try:
            ...
            self.errorWrite(f"grep: {filename}: Permission denied\n")
        except Exception:
...
class Command_tail(HoneyPotCommand):
    ...
    def tail_get_contents(self, filename: str) -> None:
        try:
            ...
            self.errorWrite(
                f"tail: cannot open `{filename}' for reading: Permission denied\n"
            )
        except Exception:
            self.errorWrite(
...
class Command_head(HoneyPotCommand):
    ...
    def head_get_file_contents(self, filename: str) -> None:
        try:
            ...
            self.errorWrite(
                f"head: cannot open `{filename}' for reading: Permission denied\n"
            )
...
class Command_cd(HoneyPotCommand):
    ...
    def call(self) -> None:
        ...
        try:
            ...
            self.errorWrite(f"bash: cd: {pname}: Permission denied\n")
            return
        except Exception:
            inode = None
        if pname == "-":
    ...
    def call(self) -> None:
        ...
        for f in args:
            ...
            try:
                ...
                self.errorWrite(f"rm: cannot remove `{f}': Permission denied\n")
                continue
...
class Command_cp(HoneyPotCommand):
    ...
    def call(self) -> None:
        ...
        try:
            dest_isdir = self.fs.isdir(resolv(dest))
            dest_exists = self.fs.exists(resolv(dest))
        except fs.PermissionDenied:
            self.errorWrite(f"cp: cannot stat `{dest}': Permission denied\n")
            return
        ...
        if dest.endswith("/") and not dest_exists and not recursive:
            self.errorWrite(
        ...
        if dest_isdir:
            ...
            try:
                parent_exists = self.fs.exists(parent)
            except fs.PermissionDenied:
                self.errorWrite(f"cp: cannot create regular file `{dest}': Permission denied\n")
                return
        ...
        for src in sources:
            try:
                src_exists = self.fs.exists(resolv(src))
                src_isdir = self.fs.isdir(resolv(src))
            except fs.PermissionDenied:
                self.errorWrite(f"cp: cannot stat `{src}': Permission denied\n")
                continue

            if not src_exists:
                self.errorWrite(f"cp: cannot stat `{src}': No such file or directory\n")
                continue
            if not recursive and src_isdir:
            ...
            try:
                ...
                # If we are using the Adaptive Deception Engine, write/copy to DB!
                dest_file_path = resolv(dest) if not isdir else os.path.join(resolv(dest), outfile)
                if hasattr(self.fs, "adaptive") and self.fs.adaptive.enabled:
                    # Perform database create
                    if src_isdir:
                        self.fs.mkdir(dest_file_path, self.protocol.user.uid, self.protocol.user.gid, 4096, s[fs.A_MODE])
                    else:
                        self.fs.mkfile(dest_file_path, self.protocol.user.uid, self.protocol.user.gid, s[fs.A_SIZE], s[fs.A_MODE])
                        # copy content
                        content = self.fs.file_contents(resolv(src))
                        self.fs.write_file_content(dest_file_path, content)
...
class Command_mv(HoneyPotCommand):
    ...
    def call(self) -> None:
        ...
        try:
            ...
            self.errorWrite(f"mv: cannot stat `{dest}': Permission denied\n")
            return
        ...
        if dest.endswith("/") and not dest_exists and len(sources) != 1:
            self.errorWrite(
        ...
        if dest_isdir:
            ...
            try:
                ...
                self.errorWrite(f"mv: cannot create regular file `{dest}': Permission denied\n")
                return
            if not parent_exists:
                self.errorWrite(
        ...
        for src in sources:
            try:
                ...
                self.errorWrite(f"mv: cannot stat `{src}': Permission denied\n")
                continue
            ...
            try:
                ...
                # If we are using Adaptive Deception Engine, call rename!
                dest_file_path = resolv(dest) if not isdir else os.path.join(resolv(dest), outfile)
                if hasattr(self.fs, "adaptive") and self.fs.adaptive.enabled:
                    self.fs.rename(resolv(src), dest_file_path)
                ...
                self.errorWrite(f"mv: cannot move `{src}` to `{dest}`: Permission denied\n")
    ...
    def call(self) -> None:
        for f in self.args:
            try:
                ...
                self.errorWrite(f"mkdir: cannot create directory `{f}': Permission denied\n")
            except fs.FileNotFound:
                self.errorWrite(
    ...
    def call(self) -> None:
        ...
        for f in self.args:
            try:
                ...
                self.errorWrite(f"touch: cannot touch `{f}`: Permission denied\n")
