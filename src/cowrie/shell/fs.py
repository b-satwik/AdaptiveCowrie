...
class HoneyPotFilesystem:
    def __getattribute__(self, name: str) -> Any:
        try:
            adaptive = object.__getattribute__(self, "adaptive")
            if adaptive and adaptive.enabled:
                fs_mgr = object.__getattribute__(self, "fs_mgr")
                
                try:
                    uid = object.__getattribute__(self, "uid")
                except AttributeError:
                    uid = 0
                try:
                    gid = object.__getattribute__(self, "gid")
                except AttributeError:
                    gid = 0
                try:
                    ...
                    username = "root"
                try:
                    profile_id = object.__getattribute__(self, "profile_id")
                except AttributeError:
                    profile_id = "default"

                adaptive.active_profile_id = profile_id
                fs_mgr.active_profile_id = profile_id
                
                fs_mgr.active_uid = uid
                fs_mgr.active_gid = gid
                fs_mgr.active_username = username
                
                if name not in ("adaptive", "fs_mgr", "__dict__", "__class__"):
                    if hasattr(fs_mgr, name):
                        return getattr(fs_mgr, name)
        except AttributeError:
            pass
        return object.__getattribute__(self, name)

    def __init__(self, arch: str, home: str, profile_id: str = "default") -> None:
        self.profile_id = profile_id
        log.msg(f"=== HoneyPotFilesystem.__init__ called (arch={arch}, home={home}, profile_id={profile_id}) ===")
        from cowrie.adaptive.engine import AdaptiveEngine
        self.adaptive = AdaptiveEngine.get_instance()
        log.msg(f"=== HoneyPotFilesystem.__init__: adaptive.enabled = {self.adaptive.enabled} ===")
        if self.adaptive.enabled:
            self.adaptive.active_profile_id = self.profile_id
            self.fs_mgr = self.adaptive.fs_mgr
            self.fs_mgr.active_profile_id = self.profile_id
            log.msg("=== HoneyPotFilesystem.__init__: calling fs_mgr.start ===")
            self.fs_mgr.start(arch, home)
            log.msg("=== HoneyPotFilesystem.__init__: fs_mgr.start finished ===")
            return
